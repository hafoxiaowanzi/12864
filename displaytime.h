...
/*往液晶屏填写 年 数据-------------------------------------------*/
	write_com(0x80);
	lcm_w_word("20");	
	year=read_clock(0x06);//调用1307时钟数据中的年数据，从地址0x8d中
	write_data(year/16+0x30);	
	write_data(year%16+0x30);
//	lcm_w_word("年");
	
/*往液晶屏填写 月 数据-------------------------------------------*/
	month=read_clock(0x05);//调用1302时钟数据中的月数据，从地址0x89中
	lcm_w_word("-");	
...
/*往液晶屏填写 日 数据-------------------------------------------*/
	date=read_clock(0x04);//从1302芯片中读取日数据，从地址0x87中
	lcm_w_word("-");
/*
	if(date/16 !=0){			  //十位消隐
...
void displayxq(void){
	...
	dd = read_clock(0x04);//日
	mo = read_clock(0x05);//月
	yy = read_clock(0x06);//年
...
/*****	显示农历月份  *****/
	...
	if (month_moon==1) {write_data(0xd5);write_data(0xfd);}
...
void welcome(void)			  //
{								  
	write_com(0x81); lcm_w_word("  欢迎使用  "); 
	write_com(0x90); lcm_w_word("  电子智能时钟  "); 	
	write_com(0x88); lcm_w_word("                "); 	
	write_com(0x98); lcm_w_word("电子制作其乐无穷");	
}
/*---------------------------------------------------------------------------*/
#endif